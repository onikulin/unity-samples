...
client.on('connect', function() {
	client.exists('wineId', function(err, reply) {
		if (reply === 0) {
//			populateDB();
		}
	});
...
exports.findById = function(req, res) {
	...
	client.hgetall('wine:' + id, function(err, object) {
		res.send(object);
	});
};

exports.findAll = function(req, res) {
	client.keys('wine:*', function(err, keys) {
...
exports.addWine = function(req, res) {
    var wine = req.body;
    if (wine.picture === null) {
    	wine.picture = "generic.jpg";
    }
    
    if (wine.year || 0 === wine.year.length) {
    	wine.year = "2012";
    }
    
    console.log('Adding wine: ' + JSON.stringify(wine));
	client.incr('wineId');
	client.get('wineId', function(err, reply) {
		wine._id = reply;
		client.hmset('wine:' + reply, wine);
		res.send(wine);
	});
};
